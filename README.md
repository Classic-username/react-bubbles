...
Set up private routes that are protected by tokens that users get by creating a login.

- [x] Describe how web servers work.

Web servers host all the information that users access. They get on a website and the website requests information from the server.

- [x] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

The post method maps to C which is Create. The get method maps to R which is Read. The put method maps to U, which is update. The Delete method maps to D.
